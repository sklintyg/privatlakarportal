...
public class MailServiceImpl implements MailService {
    ...
    private static final String BOTTOM_BODY_CONTENT = "<span><img src='cid:inera_logo'></span>";
    ...
    private MimeMessage createMessage(RegistrationStatus status, Registration registration) throws AddressException, MessagingException {
        MimeMessage message = mailSender.createMimeMessage();
        message.setSubject(SUBJECT);
        message.setFrom(new InternetAddress(from));

        message.setContent(createHtmlBody(status));

        String receiver = registration.getEpost();
        message.addRecipient(Message.RecipientType.TO,
                new InternetAddress(receiver));
        return message;
    }

    private Multipart createHtmlBody(RegistrationStatus status) throws MessagingException {
        ...
        case WAITING_FOR_HOSP:
            ...
            break;
        default:
            throw new MessagingException("Something unforseen happened");
        }
        htmlString += BOTTOM_BODY_CONTENT;
        body.setContent(htmlString, "text/html");
